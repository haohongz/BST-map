...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    // Check: the max of left subtree must be less than node
    if (node->left != nullptr) {
      if (!less(max_element_impl(node->left)->datum, node->datum)) {
        return false;
      }
    }
    // Check: node must be less than the min of right subtree
    if (node->right != nullptr) {
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    // If current node <= val, answer must be in the right subtree
    if (!less(val, node->datum)) {
      return min_greater_than_impl(node->right, val, less);
    }
    // Current node > val, so it's a candidate.
